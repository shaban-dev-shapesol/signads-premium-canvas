 ...
 * This follows Google SEO best practices:
...
export default function sitemap(): MetadataRoute.Sitemap {
  return routes.map((route) => {
    ...
    const lastmod = route.filePath ? getFileLastModified(route.filePath) : null;

    const entry: MetadataRoute.Sitemap[0] = {
      url,
    };

    // Only include lastmod if we have a valid timestamp
    if (lastmod) {
